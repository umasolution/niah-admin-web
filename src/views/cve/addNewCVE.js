...
const AddNewCVE = () => {
    ...
    const showStatus = () => {
        if (showUpdateStatus) {
            return updateStatus ? <Alert severity="success">CVE created successfully !</Alert> :
    ...
    const getNiahId = () => {
        const niahId =  "NIAH-"+cvedetails.data_type.toUpperCase()+'-'
        +cvedetails.data_id.toUpperCase();
    ...
    const updateCVEDetails = (e, key) => {
        ...
        dispatch({type:UPDATE_CVE_DETAILS, 
                data : {key : 'niahid', 
                value : getNiahIdNow(key == 'data_type' ? 1 : 2, e)}})
    }
    ...
    const getNiahIdNow = (index, e) => {
        ...
        if(index == 1 ){
            return "NIAH-"+e.target.value.toUpperCase()+'-'
                                 +cvedetails.data_id.toUpperCase();
        }else{
            return "NIAH-"+cvedetails.data_type.toUpperCase()+'-'
                ...
                <Grid container spacing={2} >
                    ...
                    <Grid item xs={10}>
                        <TextField
                            ...
                            onChange = {e=>dispatch({type:UPDATE_CVE_DETAILS, 
                                                    data : {key : 'niahid', 
                                                    value : e.target.value}})}
                    ...
                    <Grid item xs={8} className = 'react-date-wrap' style={{display:'flex',paddingTop : '0px'}}>
                        <div>
                            ...
                            <ReactDatePicker  selected={cvedetails.publisheddate != '' ? 
                                                            new Date(cvedetails.publisheddate) :
                                                                ''} 
                                          
                                            onChange = {date=>dispatch({type:UPDATE_CVE_DETAILS, 
                                                data : {key : 'publisheddate', 
                                                value :date}})}     />
                        </div>
                        
                        <div>
                            <span>Last Modified Date</span>
                            <ReactDatePicker  selected={cvedetails.lastmodifieddate != '' ? 
                                                            new Date(cvedetails.lastmodifieddate) : cvedetails.lastmodifieddate} 
