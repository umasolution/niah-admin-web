...
const CVE = () => {
    ...
    return (
        <MainCard title="Vulnerabilities">
            <SearchBar data={cve.searchCriteria} type='cve' setLoading = {setLoading}/>
            <Button variant ='contained' style={{margin:'3px'}} onClick= {onAddCVE}>Add CVE</Button>
            
            {
                isLoading ? <LinearProgress style={{ margin: "15px", width: "100%" }} /> :
                        <BasicTable columns = {cve.columns} 
                            rows={cve.rows} 
