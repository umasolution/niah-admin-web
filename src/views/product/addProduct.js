...
const AddProduct = () => {
    ...
    const onChangeNonMeta = (event, key) => {
        dispatch({ type: UPDATE_NON_DATA, data: { txt: getNiahProductId(), key: 'niah_product_id' } });
        ...
        <>
            <MainCard title="Product Details">
                <Grid container spacing={2}>
                            ...
                            if (typeof productDetails[key] == 'string') {
                                return (
                                    <TextField
                                        ...
                                        InputProps={
                                            key == 'revision' || key == 'niah_product_id'
                                                ? {
                                                      readOnly: true
                                                  }
                                                : { readOnly: false }
                    ...
                    <Grid xs={12}>
                        {Object.entries(productDetails.meta_data).map(([key, value]) => {
                            return (
                                <div style={{ display: 'flex' }}>
                                    ...
                                    <DeleteIcon
                                        ...
                                        onClick={() => {
                                            dispatch({ type: DEL_META_DATA, data: { key: key } });
                                        }}
                                    />
                                </div>
                            );
